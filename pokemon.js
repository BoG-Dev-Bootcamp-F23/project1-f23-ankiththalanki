...
async function fetchData () {
  const URL = `https://pokeapi.co/api/v2/pokemon/${
    betterMod(current_pokemon, 1017) + 1
  }/`
...
function updateData (data) {
  ...
  info_content.innerHTML =
    'height: ' +
    (Math.round(data.height * 10 + Number.EPSILON) / 100).toString() +
    'm <br>'
  info_content.innerHTML +=
    'weight: ' +
    (Math.round(data.weight * 10 + Number.EPSILON) / 100).toString() +
    'kg <br>'
  const stats = data.stats
  stats.forEach(stat => {
    info_content.innerHTML +=
      stat.stat.name + ': ' + stat.base_stat.toString() + '<br>'
  })
